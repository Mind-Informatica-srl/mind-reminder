// Package logic implements the business logic for remind
package logic
...
const (
	// ToEmailError represent an error on the destination address
	ToEmailError = "TO_MAIL_ERROR"
	// UsernameEmailError represent an error on the username
	UsernameEmailError = "USERNAME_MAIL_ERROR"
	// PasswordMailError represent an error on the password
	PasswordMailError = "PASSWORD_MAIL_ERROR"
	// ServerMailError represent an error on the server name
...
// RegisterTypes add a type in the registered types map
...
// si aggiornano tutte le righe di remind_to_calculate che hanno stesso
// object_id, object_type, created_at precedente a timeStart e elaborated_at null
func updateCorrelatedRemindToCalculate(
	db *gorm.DB,
	toCalculate *RemindToCalculate,
	timeStart time.Time,
	errorString *string,
) error {
	if err := db.Model(RemindToCalculate{}).
		Scopes(filterNotElaborated(toCalculate.ObjectID, toCalculate.ObjectType)).
		Where("created_at < ?", timeStart).
		Updates(RemindToCalculate{ElaboratedAt: &timeStart, Error: errorString}).
		Error; err != nil {
		return err
	}
	return nil
...
// RicalcolaScadenze avvia il ricalcolo delle scadenze a partire dalla
// tabella remind_to_calculate per le righe in cui la data di lavorazione è null
func RicalcolaScadenze() error {
	...
	if err := db.Transaction(func(db *gorm.DB) error {
		...
		for i := range toCalculateList {
			toCalculate := toCalculateList[i]
...
// resituisce righe da lavorare (elaborated_at is null) in ordine di data creazione decrescente
// (in modo da avere le struct in object_raw più aggiornate)
...
// (se abbiamo due righe non ancora lavorate con stesso object_id e object_type non ha
// infatti senso eseguire due volte il ricalcolo delle scadenze.
...
func getRemindToCalculate(db *gorm.DB) ([]RemindToCalculate, error) {
	...
	if err := db.Select("object_id, object_type").
		Group("object_id, object_type").
		Where("elaborated_at is null").
		Find(&list).
		Error; err != nil {
		return nil, err
	}
	for i := range list {
		...
		if err := db.Order("created_at desc").
			Scopes(filterNotElaborated(v.ObjectID, v.ObjectType)).
			First(&v).
			Error; err != nil {
			return nil, err
		}
...
// restituisce ObjectRaw (json) sotto forma di struct sfruttando
// typeRegistry per ricavare il tipo di struct da ObjectType
func getObjectFromRemindToCalculate(
	el *RemindToCalculate,
	typeRegistry map[string]reflect.Type,
) (event mrmodel.Event, err error) {
	...
	if t, ok := typeRegistry[el.ObjectType]; ok {
		...
		event, err = el.Event(t)
	} else {
		err = errors.New("Missing ObjectType " + el.ObjectType + " in typeRegistry")
	}
	return
}
...
// AddRecordRemindToCalculate Writes new reminder row to db.
